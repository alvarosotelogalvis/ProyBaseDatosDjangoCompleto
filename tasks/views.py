...
def tasks(request):
    ...
    if (len(tasks)):
        aviso = "My pending Tasks:"
    else:
        aviso = "You dont have pending tasks."
    return render(request, 'tasks.html', {'tasks':tasks, 'add': aviso})
...
def tasks_completed(request):
    ...
    if (len(tasks)):
        aviso = "My completed Tasks:"
    else:
        aviso = "You dont have completed tasks."
    return render(request, 'tasks.html', {'tasks':tasks, 'add': aviso})
...
@login_required #Protege las rutas para que se requiera un usuario logueado
def warning_delete_task(request, task_id):
    task = get_object_or_404(Task, pk=task_id, user=request.user)
    form = TaskForm(request.POST, instance = task)
    if request.method == "POST":
        #task.delete()
        #return redirect('tasks')
        #return render(request, 'warning.html', {'task':task, 'form':form})
        return render(request, 'warning_delete_task.html',{'task':task, 'form':form})
    #else:
